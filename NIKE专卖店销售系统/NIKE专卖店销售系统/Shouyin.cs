...
{
    public partial class Shouyin : Form
    {
        ...
        private void txt_barCode_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == 13)
            {
                ...
                //判断商品是否已经在购物车中存在,存在则数量+1,不存在则添加购物列表
                if (!IsExist(barCode))
                {
                    //购物列表中不存在该商品
                    //根据输入的条形码，查询商品信息
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //遍历购物车中的每一项,将其添加到销售明细表中,并更新售出商品的库存
            string sqlStr_detail = "";
            foreach (ListViewItem item in this.lv_cart.Items)
            {
                //拼接多条SQL语句,Insert销售明细在前，然后是Update商品库存，两条语句间使用"\n"分隔
        ...
        private void Shouyin_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Close();
        }
